...
@section('header', 'Edit Person')

@section('content')
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('admin.people.update', $person) }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="role" value="{{ old('role', $person->role) }}" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500">
                    @error('role')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="col-span-2">
                    ...
                    <input type="url" name="image_url" value="{{ old('image_url', $person->image_url) }}" 
