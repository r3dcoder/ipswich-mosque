...
<nav class="flex items-center space-x-2 text-sm text-gray-500 mb-6">
    ...
    <span>/</span>
    <span class="text-gray-900 font-medium">Edit {{ $menuItem->title }}</span>
</nav>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        <form action="{{ route('admin.menu-items.update', $menuItem) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <!-- Menu Group -->
                <div>
                    <label for="menu_group" class="block text-sm font-medium text-gray-700">Menu Group</label>
                    <select name="menu_group" 
                            id="menu_group"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                        <option value="services" {{ old('menu_group', $menuItem->menu_group) === 'services' ? 'selected' : '' }}>Services</option>
                        <option value="community" {{ old('menu_group', $menuItem->menu_group) === 'community' ? 'selected' : '' }}>Community</option>
                        <option value="main" {{ old('menu_group', $menuItem->menu_group) === 'main' ? 'selected' : '' }}>Main</option>
                    </select>
                    @error('menu_group')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-200">
                    ...
                    <a href="{{ route('admin.menu-items.index', ['group' => $menuItem->menu_group]) }}" 
