...
<head>
    ...
    <title>Ipswich Mosque|Prayer Time table</title>
    ...
    <meta name="author" content="Ipswich Mosque">

    <!-- Social Media / Open Graph -->
    <meta property="og:title" content="Ipswich Mosque">
    <meta property="og:description" content="Learn Quran, attend events, and support the community.">
    <meta property="og:image" content="{{ asset('images/ipswichmosque_front.jpg') }}">
    <meta property="og:url" content="https://ipswichmosque.org/">
...
@include('partials.header')
    <div class="max-w-7xl mx-auto p-6">
        ...
        <h1 class="text-2xl font-bold mb-4 text-center">Prayer Times {{ $currentMonth }} | {{ $selectedMonth }}</h1>

        <!-- Month Selector -->
        <form method="GET" action="{{ url('/prayer-times') }}" class="mb-4 text-center">
            <select name="month" onchange="this.form.submit()" class="border px-4 py-2 rounded">
        ...
        <div class="overflow-x-auto">
            <table class="w-full border-collapse border border-gray-300 text-center">
                <!-- First row: Month info -->
                <thead>
                    <tr class="bg-green-700 text-white">
                        <th colspan="13" class="py-2">
                            {{ strtoupper($selectedMonth) }} - {{ $prayerTimes->first()->hijri_month ?? '' }} -
                            {{ $prayerTimes->first()->hijri_year ?? '' }} A.H.
                        </th>
                    </tr>
                    ...
                    <tr class="bg-green-600 text-white">
                        ...
                        <th rowspan="2" class="border px-2 py-1">Day</th>
                        <th colspan="2" class="border px-2 py-1">Fajr</th>
                        <th rowspan="2" class="border px-2 py-1">Sunrise</th>
                        <th colspan="2" class="border px-2 py-1">Zuhr</th>
                        <th colspan="2" class="border px-2 py-1">Asr</th>
                        <th colspan="2" class="border px-2 py-1">Maghrib</th>
                        <th colspan="2" class="border px-2 py-1">Isha</th>
                    </tr>

                    <tr class="bg-green-500 text-white">
                        <th class="border px-2 py-1">Begins</th>
                        <th class="border px-2 py-1">Jama'at</th>
                ...
                <tbody>
                    @foreach($prayerTimes as $time)
                        <tr class="{{ $time->date == $today ? 'font-bold bg-green-100' : '' }}">
                            <td class="border px-2 py-1">{{ $time->date }}</td>
                            <td class="border px-2 py-1">{{ $time->day }}</td>
                            <td class="border px-2 py-1">{{ $time->fajr_begins }}</td>
                            <td class="border px-2 py-1">{{ $time->fajr_jamaat }}</td>
                            <td class="border px-2 py-1">{{ $time->sunrise }}</td> <!-- Sunrise -->
                            <td class="border px-2 py-1">{{ $time->zuhr_begins }}</td>
                            <td class="border px-2 py-1">{{ $time->zuhr_jamaat }}</td>
                            <td class="border px-2 py-1">{{ $time->asr_begins }}</td>
                            <td class="border px-2 py-1">{{ $time->asr_jamaat }}</td>
                            <td class="border px-2 py-1">{{ $time->maghrib_begins }}</td>
                            <td class="border px-2 py-1">{{ $time->maghrib_jamaat }}</td>
                            <td class="border px-2 py-1">{{ $time->isha_begins }}</td>
                            <td class="border px-2 py-1">{{ $time->isha_jamaat }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
